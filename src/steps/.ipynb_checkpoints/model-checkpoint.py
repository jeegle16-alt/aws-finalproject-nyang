...
BASE_COLS_CANDIDATES = [
    ...
    "gap_max", "gap_cnt_2h", "gap_long_ratio", "gap_p95", "gap_cnt_6h", "session_total_sec", "session_cnt", "step_sum",
]
...
def _preprocess(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    df["tz_lv"] = pd.to_numeric(df.get("tz_change_signal", 0).astype(object), errors="coerce").fillna(0.0)
